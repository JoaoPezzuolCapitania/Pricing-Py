"""
bond.py — Carga de informações do ativo e geração do schedule de pagamentos.
Replica fLoadBondInfo + sRunPayments + subs auxiliares do VBA.
"""
...
BOND_PATH = r"X:\#CapitaniaRFE\Trading\Ativos Capitânia"
...
class BondInfo:
    ...
    sSpreadIndexInp: str = ""
    sSpreadIndexRes: str = ""
    # Yields programadas (mudança de taxa ao longo da vida)
...
class CalcParams:
    ...
    sResult: str = ""        # "Yield", "PU", "%PUPar", "Taxa Spread", "Duration", "OverB"
    dtDay: date = None       # data de cálculo
    dYield: float = 0.0
    dSpread: float = 0.0
    dPU: float = 0.0
...
def _parse_date_bond(s):
    """Parseia data de arquivo bond (formato MM/DD/YYYY, DD/MM/YYYY ou YYYY-MM-DD)."""
    s = s.strip()
    if not s:
        return None

    # Formato YYYY-MM-DD (separador '-')
    if '-' in s:
        parts = s.split('-')
        if len(parts) != 3:
            return None
    ...
    # Formato MM/DD/YYYY ou DD/MM/YYYY (separador '/')
    if '/' not in s:
        return None
    parts = s.split('/')
    if len(parts) != 3:
    ...
    try:
        ...
        if a > 12:
            return date(c, b, a)
        # Se segundo > 12, é MM/DD/YYYY (impossível ser dia)
        if b > 12:
            return date(c, a, b)
        # Ambiguidade: assume MM/DD/YYYY (padrão US do VBA)
        return date(c, a, b)
    except (ValueError, IndexError):
        return None
...
def _load_yield(s_yield, dt_calc):
    """Parseia taxa com possíveis mudanças programadas. Replica fLoadYield."""
    ...
    # VBA retorna o PRIMEIRO yield como valor inicial (o sLoadYield atualiza depois)
    return first_yield if first_yield is not None else 0.0, dic


def load_bond(cetip, dt_calc):
    """
    Carrega informações do ativo de arquivo .txt.
    Replica fLoadBondInfo do VBA.
    Retorna: (BondInfo, CalcParams, List[PeriodInfo]) ou None se não encontrado.
    """
    dt_calc = _to_date(dt_calc)
    filepath = os.path.join(BOND_PATH, f"{cetip}.txt")
    ...
    dates_dict = {}  # ordered dict of date -> (incorp, amort, extra, multa)
    ...
    while i < len(lines):
        ...
        # Normalizar nome do campo (alguns arquivos não têm espaços)
        import re
        name = re.sub(r'\s+', ' ', name_raw)  # colapsar espaços múltiplos
        # Mapeamento sem espaços → com espaços
        _name_map = {
    ...
    # Construir PeriodInfo array (1-indexed no VBA, 0-indexed aqui)
    ...
    # Ajusta amortizações incorporadas em CDI
    dt_adjust = None
    ...
    # bPmtIncorp (fIncorpPmt)
    calc.bPmtIncorp = False

    # Error tolerance based on PU size
    if bond.dPU < 1000:
...
# ============================================================
# Lookup de curva com fallback ao proximo dia disponivel
# ============================================================

def _curve_lookup(dic, dt_wd, use_last_fallback=False):
    """
    Busca taxa na curva para dt_wd.

    Fallback quando nao encontra:
    - use_last_fallback=True (IPCA, IGPM, IGPDI):
      Se data antes do range: retorna primeiro valor do dict.
      Se data depois do range: retorna ultimo valor do dict.
      Replica VBA: dicIPCA.Items(dicIPCA.Count - 1)
    - use_last_fallback=False (DI1, CDI): retorna 0.
      Replica VBA: erro silenciado no lookup de DI1
    """
    val = dic.get(str(dt_wd))
    if val is not None:
        return val

    # Fallback
    if use_last_fallback and dic:
        return list(dic.values())[-1]
    return 0


# ============================================================
# Geração de pagamentos (sRunPayments e subs auxiliares)
# ============================================================

def _get_future(dt_from, dt_to, s_curve, calc, bond, curves):
    """Replica fGetFuture — calcula fator futuro entre duas datas."""
    ...
    d_am = 1.0
    dt_wd = brworkday(dt_to - __import__('datetime').timedelta(days=1), 1)
    ...
    if s_curve not in ("CDI", "DI1", "Pré"):
        if '+' in s_curve:
            ...
            if calc.iAMlag != 0:
                ...
                dt_wd = brworkday(dt_wd - __import__('datetime').timedelta(days=1), 1)

    # Lookup na curva
    # IPCA/IGPM/IGPDI: fallback ao ultimo valor (VBA usa dicIPCA.Items(Count-1))
    # DI1/CDI: fallback a 0 (VBA erro silenciado)
    result = 1.0
    if s_curve in ("IPCA +", "IPCA"):
        if i_days <= 0:
            result = d_am
        else:
            rate = _curve_lookup(curves.dicIPCA, dt_wd, use_last_fallback=True)
            result = d_am * (1 + rate) ** (i_days / 252)
    elif s_curve in ("IGPM +", "IGPM"):
        if i_days <= 0:
            result = d_am
        else:
            rate = _curve_lookup(curves.dicIGPM, dt_wd, use_last_fallback=True)
            result = d_am * (1 + rate) ** (i_days / 252)
    elif s_curve in ("IGPDI +", "IGPDI"):
        if i_days <= 0:
            result = d_am
        else:
            rate = _curve_lookup(curves.dicIGPDI, dt_wd, use_last_fallback=True)
            result = d_am * (1 + rate) ** (i_days / 252)
    elif s_curve in ("DI1", "CDI"):
        rate = _curve_lookup(curves.dicDI1, dt_wd)
...
def _compound_cdi(dt_from, dt_to, d_factor, calc, curves):
    """Replica fCompoundCDI — CDI acumulado entre duas datas."""
    dt_from = _to_date(dt_from)
    dt_to = _to_date(dt_to)
    i_days = brworkdays(dt_from, dt_to) - 1
...
def _get_am_principal(dt_from, dt_to, i_run, bond, calc, periods, period_bonds, curves):
    """Replica fGetAMprincipal — calcula fator de atualização monetária."""
    dt_from = _to_date(dt_from)
    dt_to = _to_date(dt_to)
    ...
    if calc.sAMmonth:
        ...
        # Correção de AM quando há eventos fora das datas
    ...
    # Ajusta datas
    if i_months == 0:
        # VBA: dtN1 = dtFrom - (Day(dtTo) - 1), dtN2 = dtTo - (Day(dtTo) - 1)
        # Subtrai Day(dtTo)-1 dias de ambas datas (NÃO é simplesmente ir pro dia 1!)
...
def run_payments(bond, calc, periods, curves):
    """
    Gera schedule de pagamentos. Replica sRunPayments.
    Retorna: List[PeriodBond]
    """
    from datetime import timedelta
    n = len(periods)
    pbs = [PeriodBond() for _ in range(n)]

    # Inicializar período 0 (mesmo que VBA: tPeriodBond(1))
    pbs[0].dSN = bond.dPU
    pbs[0].dSNA = bond.dPU
    pbs[0].dFatAmAcc = 1.0

    for i in range(1, n):
        # sLoadYield — verifica se taxa muda neste período
        if bond.dicYields:
...
def _calc_yield(i, bond, calc, periods, pbs, curves):
    """Replica sYield — calcula fatores de juros para o período."""
    ...
    # Yield spread (fator de juros do período)
    if bond.sIndex != "% CDI":
...
def _get_period_days(i, calc, periods):
    """Calcula dias do período conforme convenção."""
    if calc.sYdays == "Úteis":
        i_days = brworkdays(periods[i - 1].dtDay, periods[i].dtDay) - 1
        return i_days, 0
...
def _calc_am(i, bond, calc, periods, pbs, curves):
    """Replica sAM — atualização monetária."""
...
def _calc_pmt(i, bond, calc, periods, pbs):
    """Replica sPMT — cálculo de pagamentos."""
    if not calc.bAmort100:
        # Incorporated yield
        if periods[i].dIncorpYield != 0:
            if periods[i].dIncorpYield <= 1:
    ...
    # Extra amort e multa (somente após ex-date)
...
def _calc_pv_par(i, bond, calc, periods, pbs):
    """Replica sPvPar — valor presente para cálculo do par."""
